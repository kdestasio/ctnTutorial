    ...
    for line in lines:
        ...
        if os.path.isdir(subjectpath):
            ...
            if cfg.run_local:
                ...
                batch_cmd = 'module load singularity; sbatch --job-name dcm2bids_{subjectdir} -A {account} --partition=ctn --time 00:60:00 --mem-per-cpu=2G --cpus-per-task=1 -o {logdir}/{subjectdir}_dcm2bids_output.txt -e {logdir}/{subjectdir}_dcm2bids_error.txt --wrap="sleep {sleeptime}; singularity exec -B {path_dicoms} -B {path_bidsdata} -B {path_conversionfolder} {singularity_image} dcm2bids -d {subjectpath} -s {wave} -p {subject} -c {path_config} -o {path_bidsdata}  --forceDcm2niix --clobber"'.format(logdir=cfg.logdir, subjectdir=subjectdir, path_dicoms=cfg.path_dicoms, wave=wave, path_conversionfolder=cfg.path_conversionfolder, path_config=cfg.path_config, subject=subject, path_bidsdata=cfg.path_bidsdata, subjectpath=subjectpath, singularity_image=cfg.singularity_image, account=cfg.group, sleeptime=sleeptime)
